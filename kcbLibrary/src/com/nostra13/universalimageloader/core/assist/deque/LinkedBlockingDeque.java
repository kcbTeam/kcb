 ...
 * The capacity, if unspecified, is equal to {@link Integer#MAX_VALUE}. Linked nodes are dynamically
 ...
 * {@link #removeLastOccurrence removeLastOccurrence}, {@link #contains contains}, {@link #iterator
...
public class LinkedBlockingDeque<E> extends AbstractQueue<E>
    ...
    /*
     * Implemented as a simple doubly-linked list protected by a single lock and using conditions to
     * manage blocking.
     * 
     ...
     * if a Node was tenured while live, which generational GCs have a hard time dealing with,
     * causing repeated major collections. However, only non-deleted Nodes need to be reachable from
     * dequeued Nodes, and reachability does not necessarily have to be of the kind understood by
     * the GC. We use the trick of linking a Node that has just been dequeued to itself. Such a
     * self-link implicitly means to jump to "first" (for next links) or "last" (for prev links).
     */

    /*
     * We have "diamond" multiple interface/abstract class inheritance here, and that introduces
     * ambiguities. Often we want the BlockingDeque javadoc combined with the AbstractQueue
     ...
     * restrictions. When using a capacity-restricted deque, it is generally preferable to use
     * method {@link #offer offer}.
     * 
     * <p>
     ...
     * 
     * We don't want to acquire the lock for every iteration, but we also want other threads a
     * chance to interact with the collection, especially when count is close to capacity.
     */
    ...
    /**
     * Returns an array containing all of the elements in this deque, in proper sequence; the
     * runtime type of the returned array is that of the specified array. If the deque fits in the
     ...
     * 
     * <p>
     * Suppose {@code x} is a deque known to contain only strings. The following code can be used to
     * dump the deque into a newly allocated array of {@code String}:
     * 
     * <pre>
     ...
     * 
     * @param a the array into which the elements of the deque are to be stored, if it is big
