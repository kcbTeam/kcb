...
public final class ImageSizeUtils {
     ...
     * {@link com.nostra13.universalimageloader.core.imageaware.ImageAware view} parameters,
    ...
    /**
     * Computes sample size for downscaling image size (<b>srcSize</b>) to view size
     * (<b>targetSize</b>). This sample size is used during
     ...
     * width/height of the original, and 1/16 the number of pixels. Any value <= 1 is treated the
     * same as 1.
     * 
    ...
    /**
     * Computes minimal sample size for downscaling image so result image size won't exceed max
     * acceptable OpenGL texture size.<br />
     * We can't create Bitmap in memory with size exceed max texture size (usually this is
     * 2048x2048) so this method calculate minimal sample size which should be applied to image to
     * fit into these limits.
     * 
     * @param srcSize Original image size
     ...
     * @param targetSize Target (view) size
     * @param viewScaleType {@linkplain ViewScaleType Scale type} for placing image in view
     * @param stretch Whether source size should be stretched if target size is larger than source
     *        size. If <b>false</b> then result scale value can't be greater than 1.
