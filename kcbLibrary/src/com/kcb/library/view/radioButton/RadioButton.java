package com.kcb.library.view.radioButton;
...
/**
 * Created by IntelliJ IDEA. User: keith. Date: 14-10-8. Time: 15:48.
 */
public class RadioButton extends CompoundButton {
	...
	public RadioButton(Context context, AttributeSet attrs, int defStyle) {
		...
		mCheckedColor = attributes.getColor(
				R.styleable.RadioButton_radio_checked_color, getResources()
						.getColor(R.color.radio_checked_color));
		attributes.recycle();
		mRadioWidth = getResources().getDimensionPixelSize(R.dimen.radio_width);
		mRadioHeight = getResources().getDimensionPixelSize(
				R.dimen.radio_height);
		mBorderRadius = getResources().getDimensionPixelSize(
				R.dimen.radio_border_radius);
		mThumbRadius = getResources().getDimensionPixelSize(
				R.dimen.radio_thumb_radius);
		mRippleRadius = getResources().getDimensionPixelSize(
				R.dimen.radio_ripple_radius);
		mStrokeWidth = getResources().getDimensionPixelSize(
				R.dimen.radio_stroke_width);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		widthMeasureSpec = MeasureSpec.makeMeasureSpec(
				measureWidth(widthMeasureSpec), MeasureSpec.EXACTLY);
		heightMeasureSpec = MeasureSpec.makeMeasureSpec(
				measureHeight(heightMeasureSpec), MeasureSpec.EXACTLY);
	...
	protected void onDraw(@NonNull Canvas canvas) {
		...
		case StateTouchDown:
			...
			if (elapsed < ANIMATION_DURATION) {
				rippleRadius = Math.round(elapsed * mRippleRadius
						/ ANIMATION_DURATION);
			} else {
		...
		case StateTouchUp:
			if (elapsed < ANIMATION_DURATION) {
				...
				rippleRadius = Math.round((ANIMATION_DURATION - elapsed)
						* mRippleRadius / ANIMATION_DURATION);
				if (isChecked()) {
					thumbRadius = Math.round(elapsed * mThumbRadius
							/ ANIMATION_DURATION);
				} else {
					thumbRadius = Math.round((ANIMATION_DURATION - elapsed)
							* mThumbRadius / ANIMATION_DURATION);
				}
			} else {
				mState = StateNormal;
				rippleRadius = 0;
		...
		canvas.drawCircle(getWidth() / 2, getHeight() / 2, rippleRadius,
				ripplePaint);
		canvas.drawCircle(getWidth() / 2, getHeight() / 2, mBorderRadius,
				borderPaint);
		canvas.drawCircle(getWidth() / 2, getHeight() / 2, thumbRadius,
				thumbPaint);
	}
