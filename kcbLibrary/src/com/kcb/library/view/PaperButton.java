...
public class PaperButton extends View {
    ...
    public PaperButton(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mColor =
                attributes.getColor(R.styleable.PaperButton_paper_color,
                        getResources().getColor(R.color.paper_button_color));
        mShadowColor =
                attributes.getColor(R.styleable.PaperButton_paper_shadow_color, getResources()
                        .getColor(R.color.paper_button_shadow_color));
        mCornerRadius =
                attributes.getDimensionPixelSize(R.styleable.PaperButton_paper_corner_radius,
        ...
        mTextSize =
                attributes.getDimensionPixelSize(R.styleable.PaperButton_paper_text_size,
                        getResources().getDimensionPixelSize(R.dimen.paper_text_size));
        mTextColor =
                attributes.getColor(R.styleable.PaperButton_paper_text_color, getResources()
        ...
        mShadowRadius =
                attributes.getFloat(R.styleable.PaperButton_paper_shadow_radius, SHADOW_RADIUS);
        mShadowOffsetX =
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        switch (mState) {
            ...
            case StateTouchDown:
                ...
                if (elapsed < ANIMATION_DURATION) {
                    ...
                    float shadowAlpha =
                            (MAX_SHADOW_COLOR_ALPHA - MIN_SHADOW_COLOR_ALPHA) * elapsed
                                    / ANIMATION_DURATION + MIN_SHADOW_COLOR_ALPHA;
            ...
            case StateTouchUp:
                if (elapsed < ANIMATION_DURATION) {
                    int alpha =
                            Math.round((ANIMATION_DURATION - elapsed) * 255 / ANIMATION_DURATION);
                    ripplePaint.setAlpha(alpha);
                    radius =
                            getWidth() / 2
                                    + Math.round(elapsed * getWidth() / 2 / ANIMATION_DURATION);
                    float shadowAlpha =
                            (MAX_SHADOW_COLOR_ALPHA - MIN_SHADOW_COLOR_ALPHA)
                                    * (ANIMATION_DURATION - elapsed) / ANIMATION_DURATION
        ...
        if (mState == StateTouchDown || mState == StateTouchUp) {
            if (rippleClipPath == null) {
                ...
                rippleClipPath.addRoundRect(getRectF(), mCornerRadius, mCornerRadius,
                        Path.Direction.CW);
            }
