package com.kcb.library.view.smoothprogressbar;

import static com.kcb.library.view.smoothprogressbar.SmoothProgressBarUtils.checkColors;
import static com.kcb.library.view.smoothprogressbar.SmoothProgressBarUtils.checkNotNull;
import static com.kcb.library.view.smoothprogressbar.SmoothProgressBarUtils.checkPositive;
import static com.kcb.library.view.smoothprogressbar.SmoothProgressBarUtils.checkPositiveOrZero;
import static com.kcb.library.view.smoothprogressbar.SmoothProgressBarUtils.checkSpeed;
...
public class SmoothProgressDrawable extends Drawable implements Animatable {
    ...
    private SmoothProgressDrawable(Interpolator interpolator, int sectionsCount,
            int separatorLength, int[] colors, float strokeWidth, float speed,
            float progressiveStartSpeed, float progressiveStopSpeed, boolean reversed,
            boolean mirrorMode, Callbacks callbacks, boolean progressiveStartActivated,
    ...
    private void drawGradient(Canvas canvas) {
        ...
        float left =
                mReversed ? (mMirrorMode
                        ? Math.abs(mBounds.left - mBounds.right) / 2
                        : mBounds.left) : mBounds.left;
        float right =
                mMirrorMode ? (mReversed
                        ? mBounds.left
                        : Math.abs(mBounds.left - mBounds.right) / 2) : mBounds.right;
        ...
        LinearGradient linearGradient =
                new LinearGradient(left, top, right, bottom, mLinearGradientColors,
                        mLinearGradientPositions, mMirrorMode
    ...
    private void drawStrokes(Canvas canvas) {
        ...
        for (int i = 0; i <= mCurrentSections; ++i) {
            ...
            ratioSectionWidth =
                    Math.abs(mInterpolator.getInterpolation(prev)
            ...
            if (end > prevValue && i >= mStartSection) {
                float xFinishingOffset =
                        mInterpolator.getInterpolation(Math.min(mFinishingOffset, 1f));
    ...
    public static class Builder {
        ...
        public SmoothProgressDrawable build() {
            ...
            SmoothProgressDrawable ret =
                    new SmoothProgressDrawable(mInterpolator, mSectionsCount,
                            mStrokeSeparatorLength, mColors, mStrokeWidth, mSpeed,
                            mProgressiveStartSpeed, mProgressiveStopSpeed, mReversed, mMirrorMode,
                            mOnProgressiveStopEndedListener, mProgressiveStartActivated,
