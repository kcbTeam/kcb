...
public class EditTestDialog extends android.app.Dialog {
	...
	public void dismiss() {
		...
		Animation backAnim = AnimationUtils.loadAnimation(mContext,
				R.anim.dialog_root_hide);
	 ...
	 * step 3
	 */
	public void setSureButton(@NonNull int resid,
			final DialogBackListener listener) {
		setSureButton(mContext.getResources().getString(resid), listener);
	}

	public void setSureButton(@NonNull CharSequence text,
			final DialogBackListener listener) {
		...
		sureButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				dismiss();
				if (null != listener) {
					listener.refreshActivity(contentEditText.getText()
							.toString());
				}
			}
		});
