...
public class EditTestActivity extends BaseActivity {
	...
	protected void initView() {
		...
		numHintTextView.setText(String.format(
				getResources().getString(R.string.format_question_num),
				mCurrentPosition + 1));
	...
	public void onClick(View v) {
		...
		case R.id.edittext_A:
			...
			makeEditDialog(optionAEditText.getText().toString(), getResources()
					.getString(R.string.dialog_title_optionA));
			break;
		case R.id.edittext_B:
			mPositionIndex = IndexOfB;
			makeEditDialog(optionBEditText.getText().toString(), getResources()
					.getString(R.string.dialog_title_optionB));
			break;
		case R.id.edittext_C:
			mPositionIndex = IndexOfC;
			makeEditDialog(optionCEditText.getText().toString(), getResources()
					.getString(R.string.dialog_title_optionC));
			break;
		case R.id.edittext_D:
			mPositionIndex = IndexOfD;
			makeEditDialog(optionDEditText.getText().toString(), getResources()
					.getString(R.string.dialog_title_optionD));
			break;
		default:
			break;
		}
	...
	private void makeEditDialog(String text, String title) {
		...
		dialog.setSureButton(getResources().getString(R.string.save),
				mSureClickListener);
				...
				if (!mQuestionList.get(i).isLegal()) {
					ToastUtil.toast(String.format(
							getResources().getString(
									R.string.format_edit_empty_hint), 1 + i));
					return;
				}
			}
			completeEdit();
	...
	private void clickDelete() {
		if (MaxFragmentNum > 1) {
			...
			DialogUtil.showNormalDialog(this, R.string.leave,
					R.string.sureLeave, R.string.sure, sureListener,
	...
	private ChoiceQuestion getCurrentObj() {
		...
		boolean[] correctOption = { checkBoxA.isCheck(), checkBoxB.isCheck(),
				checkBoxC.isCheck(), checkBoxD.isCheck() };
		return new ChoiceQuestion(question, optionA, optionB, optionC, optionD,
				correctOption);
	}
